...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    #print(typed_words, source_words)
    count = 0
    # END PROBLEM 3
...
def feline_fixes(typed, source, limit):
    ...
    return feline_fixes(typed[1:], source[1:], limit-diff) + diff
...
def minimum_mewtations(typed, source, limit):
    ...
    if typed[0]==source[0]: # Feel free to remove or add additional cases
        ...
        add = minimum_mewtations(typed, source[1:], limit-diff) +diff# Fill in these lines
        remove = minimum_mewtations(typed[1:], source, limit-diff)+diff
        substitute = minimum_mewtations(typed[1:], source[1:], limit-diff)+diff
        # BEGIN
        "*** YOUR CODE HERE ***"
...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] == source[i]:
            ...
            break
    progress = count/max(len(typed), len(source))
    res["id"] = user_id
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in range(len(timestamps_per_player)):
        ...
        for j in range(1,len(timestamps_per_player[i])):
            temp.append(timestamps_per_player[i][j]-timestamps_per_player[i][j-1])
        timestamps_per_player[i][:]=temp
...
def fastest_words(match):
    ...
    for i in player_indices:
        res.append([])
    transpose = [[times[j][i] for j in range(len(times))] for i in range(len(times[0]))]


    for i in range(len(transpose)):#3
        for j in range(len(transpose[i])):#2
            if transpose[i][j] == min(transpose[i]):
